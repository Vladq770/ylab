...
class PostService(ServiceMixin):
    ...
    def create_post(self, post: PostCreate) -> dict:
        ...
        if user is None or user.last_login + 300 < int(
                datetime.utcnow().timestamp()):  # +300: истек ли access token
            return dict()
        if post.accessToken != encode({"username": user.username, "time": user.last_login}, JWT_SECRET_KEY,
